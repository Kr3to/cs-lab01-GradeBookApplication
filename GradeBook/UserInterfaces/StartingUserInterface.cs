...
namespace GradeBook.UserInterfaces
{
    public static class StartingUserInterface
    {
        ...
        public static void CreateCommand(string command)
        {
            ...
            {
                BaseGradeBook gradeBook = new StandardGradeBook(name, isWeighted: true);
                GradeBookUserInterface.CommandLoop(gradeBook);
            }
            else if (parts[2] == "ranked" && parts[3] == "true")
            {
                BaseGradeBook gradeBook = new RankedGradeBook(name, isWeighted: true);
                GradeBookUserInterface.CommandLoop(gradeBook);
            }
            else if (parts[2] == "standard" && parts[3] == "false")
            {
                BaseGradeBook gradeBook = new RankedGradeBook(name, isWeighted: false);
                GradeBookUserInterface.CommandLoop(gradeBook);
            }
